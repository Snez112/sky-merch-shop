...
export default function Header() {
    ...
    if (!mounted) {
        return (
             <header className="sticky top-0 z-50 glass-nav border-b border-[#f4e7e7] dark:border-[#1e3a5f] px-6 lg:px-40 py-4 font-display">
                <div className="max-w-[1200px] mx-auto flex items-center justify-between">
                     ...
                     <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-lg md:text-xl font-black tracking-tight text-primary uppercase">TIM SIÊU NHANH</h2>
                    </div>
                </div>
            </header>
        )
    ...
    return (
        <header className="sticky top-0 z-50 glass-nav border-b border-[#f4e7e7] dark:border-[#1e3a5f] px-6 lg:px-40 py-4 font-display">
            ...
            {/* Mobile Menu */}
            {isMenuOpen && (
                <div className="md:hidden absolute top-full left-0 right-0 glass-nav border-b border-[#f4e7e7] dark:border-[#1e3a5f] p-6 flex flex-col gap-4 shadow-xl">
                    {menuItems.map((item) => (
                        <Link
                            key={item.label}
                            href={item.href}
